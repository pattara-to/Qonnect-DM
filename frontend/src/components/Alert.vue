...
onMounted(() => {
    statuses.value = props.alert.status.split("");
});

watch(
    () => props.alert,
    (newAlert) => {
        statuses.value = newAlert.status.split("");
    }
);
...
const editAlert = async () => {
    ...
    try {
        const response = await props.editAlert(props.alert.id, alertData);
...
<template>
    <div class="flex flex-wrap w-full h-full justify-around items-center space-y-2 sm:space-y-0">
        ...
        <div class="text-lg w-16 flex justify-around">
            ...
            <button @click="props.removeAlert(props.alert.id)"><img src="../assets/DeleteIcon.svg" class="w-5 hover:scale-105" /></button>
        </div>
    </div>
</template>
<style scoped>
